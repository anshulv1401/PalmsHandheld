//// Warning: Do not modify!
//// This file (PartBatchEntity.cs) has been auto-generated by PALMS Entity Generation Tool on '18 September 2014' at '12:03:04'
//// If modification is required, consider 'partial class'.

using System;
using System.Collections;
using System.Collections.Generic;
using Technoforte.PALMS.Entities.Views.PrimaryKeys;
using Technoforte.PALMS.Entities.Views.UniqueKeys;

#pragma warning disable 1591

namespace Technoforte.PALMS.Entities.Views
{
	[Serializable]
	public partial class PartBatchEntity : Entity
	{
		#region Enums

		public new enum Field
		{
			PrimaryCompanyID
			, WarehouseIDOfGoodsReceivingNote
			, CostBucketIDOfGoodsReceivingNote
			, PartID
			, PartBatchID
			, PartBatchNo
			, ManufacturingDate
			, ExpiryPeriod
			, ExpiryPeriodUnit
			, ExpiryDate
			, SerialNo
			, GoodsReceivingNoteID
			, GoodsReceivingNoteLineNo
			, CostBucketIDOfGoodsReceivingNote_GoodsReceivingNoteNo
			, GoodsReceivingNoteID_GoodsReceivingNoteNo
			, WarehouseIDOfGoodsReceivingNote_GoodsReceivingNoteNo
			, CreatedBy_PalmsUserLoginName
			, ModifiedBy_PalmsUserLoginName
			, PartID_PartCode
			, PartID_PartName
			, PrimaryCompanyID_PrimaryCompanyCode
			, PrimaryCompanyID_PrimaryCompanyName
			
			, _FieldCount
		}

		public enum UniqueKey
		{
			No
		}

		#endregion Enums

		#region Members

		public int? primaryCompanyID;
		public int? warehouseIDOfGoodsReceivingNote;
		public int? costBucketIDOfGoodsReceivingNote;
		public int? partID;
		public int? partBatchID;
		public string partBatchNo;
		public DateTime? manufacturingDate;
		public int? expiryPeriod;
		public int? expiryPeriodUnit;
		public DateTime? expiryDate;
		public string serialNo;
		public int? goodsReceivingNoteID;
		public int? goodsReceivingNoteLineNo;
		public string costBucketIDOfGoodsReceivingNote_GoodsReceivingNoteNo;
		public string goodsReceivingNoteID_GoodsReceivingNoteNo;
		public string warehouseIDOfGoodsReceivingNote_GoodsReceivingNoteNo;
		public string createdBy_PalmsUserLoginName;
		public string modifiedBy_PalmsUserLoginName;
		public string partID_PartCode;
		public string partID_PartName;
		public string primaryCompanyID_PrimaryCompanyCode;
		public string primaryCompanyID_PrimaryCompanyName;

		#endregion Members

		#region Overriding / Public Members

		public override IEntityKey<Entity> GetPrimaryKey()
		{
			return (IEntityKey<Entity>)new PartBatchPrimaryKey()
			{
				partBatchID = (int)partBatchID
				, primaryCompanyID = (int)primaryCompanyID
			};
		}

		public override IEntityKey<Entity> GetBlankPrimaryKey()
		{
			return (IEntityKey<Entity>)new PartBatchPrimaryKey();
		}

		public override IEntityKey<Entity> GetUniqueKey(string uniqueKey)
		{
			if (uniqueKey == "")
			{
				uniqueKey = "No";
			}

			return (IEntityKey<Entity>)GetUniqueKey((UniqueKey)Enum.Parse(typeof(UniqueKey), uniqueKey, false));
		}

		public EntityKey<PartBatchEntity> GetUniqueKey(UniqueKey uniqueKey)
		{
			switch (uniqueKey)
			{
				case UniqueKey.No:
					return new PartBatchUniqueKey_No()
					{
						expiryDate = (expiryDate == null ? (DateTime?)expiryDate : DateTime.SpecifyKind((DateTime)expiryDate, DateTimeKind.Local))
						, goodsReceivingNoteID = (int?)goodsReceivingNoteID
						, manufacturingDate = (manufacturingDate == null ? (DateTime?)manufacturingDate : DateTime.SpecifyKind((DateTime)manufacturingDate, DateTimeKind.Local))
						, partBatchNo = (string)partBatchNo
						, partID = (int?)partID
						, primaryCompanyID = (int?)primaryCompanyID
						, serialNo = (string)serialNo
					};
			}

			return null;
		}

		public override IEntityKey<Entity> GetBlankUniqueKey(string uniqueKey )
		{
			if (uniqueKey == "")
			{
				uniqueKey = "No";
			}

			return (IEntityKey<Entity>)GetBlankUniqueKey((UniqueKey)Enum.Parse(typeof(UniqueKey), uniqueKey, false));
		}

		public EntityKey<PartBatchEntity> GetBlankUniqueKey(UniqueKey uniqueKey)
		{
			switch (uniqueKey)
			{
				case UniqueKey.No:
					return new PartBatchUniqueKey_No();
			}

			return null;
		}

		public override int GetFieldCount()
		{
			return (int)Field._FieldCount;
		}

		public override object GetValue(int field)
		{
			return GetValue((Field)field);
		}

		public object GetValue(Field field)
		{
			switch (field)
			{
				case Field.PrimaryCompanyID:
					return primaryCompanyID;
				case Field.WarehouseIDOfGoodsReceivingNote:
					return warehouseIDOfGoodsReceivingNote;
				case Field.CostBucketIDOfGoodsReceivingNote:
					return costBucketIDOfGoodsReceivingNote;
				case Field.PartID:
					return partID;
				case Field.PartBatchID:
					return partBatchID;
				case Field.PartBatchNo:
					return partBatchNo;
				case Field.ManufacturingDate:
					return manufacturingDate;
				case Field.ExpiryPeriod:
					return expiryPeriod;
				case Field.ExpiryPeriodUnit:
					return expiryPeriodUnit;
				case Field.ExpiryDate:
					return expiryDate;
				case Field.SerialNo:
					return serialNo;
				case Field.GoodsReceivingNoteID:
					return goodsReceivingNoteID;
				case Field.GoodsReceivingNoteLineNo:
					return goodsReceivingNoteLineNo;
				case Field.CostBucketIDOfGoodsReceivingNote_GoodsReceivingNoteNo:
					return costBucketIDOfGoodsReceivingNote_GoodsReceivingNoteNo;
				case Field.GoodsReceivingNoteID_GoodsReceivingNoteNo:
					return goodsReceivingNoteID_GoodsReceivingNoteNo;
				case Field.WarehouseIDOfGoodsReceivingNote_GoodsReceivingNoteNo:
					return warehouseIDOfGoodsReceivingNote_GoodsReceivingNoteNo;
				case Field.CreatedBy_PalmsUserLoginName:
					return createdBy_PalmsUserLoginName;
				case Field.ModifiedBy_PalmsUserLoginName:
					return modifiedBy_PalmsUserLoginName;
				case Field.PartID_PartCode:
					return partID_PartCode;
				case Field.PartID_PartName:
					return partID_PartName;
				case Field.PrimaryCompanyID_PrimaryCompanyCode:
					return primaryCompanyID_PrimaryCompanyCode;
				case Field.PrimaryCompanyID_PrimaryCompanyName:
					return primaryCompanyID_PrimaryCompanyName;
			}

			return null;
		}

		public override string GetFieldName(int field)
		{
			return GetFieldName((Field)field);
		}

		public string GetFieldName(Field field)
		{
			return field.ToString();
		}

		public override Type GetType(int field)
		{
			return GetType((Field)field);
		}

		public Type GetType(Field field)
		{
			switch (field)
			{
				case Field.PrimaryCompanyID:
					return typeof(int?);
				case Field.WarehouseIDOfGoodsReceivingNote:
					return typeof(int?);
				case Field.CostBucketIDOfGoodsReceivingNote:
					return typeof(int?);
				case Field.PartID:
					return typeof(int?);
				case Field.PartBatchID:
					return typeof(int?);
				case Field.PartBatchNo:
					return typeof(string);
				case Field.ManufacturingDate:
					return typeof(DateTime?);
				case Field.ExpiryPeriod:
					return typeof(int?);
				case Field.ExpiryPeriodUnit:
					return typeof(int?);
				case Field.ExpiryDate:
					return typeof(DateTime?);
				case Field.SerialNo:
					return typeof(string);
				case Field.GoodsReceivingNoteID:
					return typeof(int?);
				case Field.GoodsReceivingNoteLineNo:
					return typeof(int?);
				case Field.CostBucketIDOfGoodsReceivingNote_GoodsReceivingNoteNo:
					return typeof(string);
				case Field.GoodsReceivingNoteID_GoodsReceivingNoteNo:
					return typeof(string);
				case Field.WarehouseIDOfGoodsReceivingNote_GoodsReceivingNoteNo:
					return typeof(string);
				case Field.CreatedBy_PalmsUserLoginName:
					return typeof(string);
				case Field.ModifiedBy_PalmsUserLoginName:
					return typeof(string);
				case Field.PartID_PartCode:
					return typeof(string);
				case Field.PartID_PartName:
					return typeof(string);
				case Field.PrimaryCompanyID_PrimaryCompanyCode:
					return typeof(string);
				case Field.PrimaryCompanyID_PrimaryCompanyName:
					return typeof(string);

			}
			return null;
		}

		public override void SetValue(int field, bool? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, bool? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, long? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, long? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, int? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, int? value)
		{
			switch (field)
			{
				case Field.PrimaryCompanyID:
					primaryCompanyID = value;
					break;
				case Field.WarehouseIDOfGoodsReceivingNote:
					warehouseIDOfGoodsReceivingNote = value;
					break;
				case Field.CostBucketIDOfGoodsReceivingNote:
					costBucketIDOfGoodsReceivingNote = value;
					break;
				case Field.PartID:
					partID = value;
					break;
				case Field.PartBatchID:
					partBatchID = value;
					break;
				case Field.ExpiryPeriod:
					expiryPeriod = value;
					break;
				case Field.ExpiryPeriodUnit:
					expiryPeriodUnit = value;
					break;
				case Field.GoodsReceivingNoteID:
					goodsReceivingNoteID = value;
					break;
				case Field.GoodsReceivingNoteLineNo:
					goodsReceivingNoteLineNo = value;
					break;
				default:
					break;
			}
		}

		public override void SetValue(int field, short? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, short? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, byte? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, byte? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, BitArray value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, BitArray value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, float? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, float? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, double? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, double? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, decimal? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, decimal? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, char? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, char? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override void SetValue(int field, string value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, string value)
		{
			switch (field)
			{
				case Field.PartBatchNo:
					partBatchNo = value;
					break;
				case Field.SerialNo:
					serialNo = value;
					break;
				case Field.CostBucketIDOfGoodsReceivingNote_GoodsReceivingNoteNo:
					costBucketIDOfGoodsReceivingNote_GoodsReceivingNoteNo = value;
					break;
				case Field.GoodsReceivingNoteID_GoodsReceivingNoteNo:
					goodsReceivingNoteID_GoodsReceivingNoteNo = value;
					break;
				case Field.WarehouseIDOfGoodsReceivingNote_GoodsReceivingNoteNo:
					warehouseIDOfGoodsReceivingNote_GoodsReceivingNoteNo = value;
					break;
				case Field.CreatedBy_PalmsUserLoginName:
					createdBy_PalmsUserLoginName = value;
					break;
				case Field.ModifiedBy_PalmsUserLoginName:
					modifiedBy_PalmsUserLoginName = value;
					break;
				case Field.PartID_PartCode:
					partID_PartCode = value;
					break;
				case Field.PartID_PartName:
					partID_PartName = value;
					break;
				case Field.PrimaryCompanyID_PrimaryCompanyCode:
					primaryCompanyID_PrimaryCompanyCode = value;
					break;
				case Field.PrimaryCompanyID_PrimaryCompanyName:
					primaryCompanyID_PrimaryCompanyName = value;
					break;
				default:
					break;
			}
		}

		public override void SetValue(int field, DateTime? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, DateTime? value)
		{
			switch (field)
			{
				case Field.ManufacturingDate:
					manufacturingDate = value;
					break;
				case Field.ExpiryDate:
					expiryDate = value;
					break;
				default:
					break;
			}
		}

		public override void SetValue(int field, Guid? value)
		{
			SetValue((Field)field, value);
		}

		public void SetValue(Field field, Guid? value)
		{
			switch (field)
			{
				default:
					break;
			}
		}

		public override bool IsIdentity(int field)
		{
			if ((Field)field == Field.PartBatchID)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		#endregion Overriding / Public Members
	}
}
